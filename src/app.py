...
from stats.negative_stock_expected_value import logistics_index
from connections import operations_weekly, price
...
def return_order(n_clicks, item, store, depth):
    ...
    return html.Div(children=[
        ...
        html.Div(
            html.H2('-E{Zt|Zt < 0} = ' + f'{be}'),
            style={'backgroundColor': '#FFAB33', 'pading': 2, 'marging': 2}
        ),
        html.Div(
            html.H2(f'P{{0 < Zt < {A}}} = ' + f'{bp}'),
            style={'backgroundColor': '#33FF8B', 'pading': 2, 'marging': 2}
        )
    ])
