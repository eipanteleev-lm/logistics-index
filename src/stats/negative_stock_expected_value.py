...
def deep_next(y: Union[int, float], z: Union[int, float], d: Dict[Union[int, float], Callable], depth: int, **kwargs) -> Union[int, float]:
    ...
    return deep_next(y, next(0, z, d, **kwargs), d, depth - 1)
...
def find_best_solution(z: Union[int, float], d: Dict[Union[int, float], Callable], bounds:Tuple[Union[int, float]], depth:int=0, **kwargs) -> Union[int, float]:
    """
    :param z: int or float, the current stock level
    ...
    for y in range(0, int(bounds[1]) + 2, 2):
        e, p = negative_expected_value(y, z, bounds, d, 500, depth, **kwargs)
        if bp < 0.9:
            if p > bp:
                be, bp, by = e, p, y
